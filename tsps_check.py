...
xmlFile = 'tsps_status.xml'
servers = [['ocvlp-bmc004', 'https://ocvlp-bmc004.prod.oami.eu:8043'], ['orvlp-bmc404', 'https://orvlp-bmc404.prod.oami.eu:8043']]

root = ET.Element('tsps_status')
comment = ET.Comment('This XML is generated by a script called tsps_check.py at ocvlp-bmc014. In case of malfunction or further changes check the script there.')
...
for server in servers:
	...
	ET.SubElement(alert, 'name').text = server[0]
	ET.SubElement(alert, 'role').text = server[2]
	ET.SubElement(alert, 'response_code').text = str(server[3])
	if (server[0] == 'ocvlp-bmc004' and server[2] == 'active') or \
		 (server[0] == 'orvlp-bmc404' and server[2] == 'standby'):
